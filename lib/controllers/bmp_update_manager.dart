...
class BmpUpdateManager {
  ...
  Future<bool> updateBmp(String lr, Uint8List image, {int? seq}) async {
    ...
    print("BmpUpdate -> updateBmp: start sending ${multiPacks.length} packs, image length: ${image.length} *************************");
    ...
    for (int index = 0; index < multiPacks.length; index++) { 
      ...
      print("${DateTime.now()} updateBmp----data---*${data.length}------------");
...
extension MultiBmpUpdate on BmpUpdateManager {
  Future<bool> updateTheBmp(String lr, Uint8List image, {int? seq}) async {

    // check if has error sending package
    bool isOldSendPackError(int? currentSeq) {
      bool oldSendError = (seq == null && currentSeq != null);
      if (oldSendError) {
        print("MultiBmpUpdate -> updateTheBmp: old pack send error, seq = $currentSeq");
      }
      return oldSendError;
    }
    ...
    for (int index = 0; index < multiPacks.length; index++) { 
      ...
      // 4 字节长度变量（可修改）
      int lengthValue = 0x1c; // 28（示例值）
  
      // 计算前缀
      List<int> prefix = index == 0 
      ? [0x15, index & 0xff, ...Utils.intToBytes(lengthValue)]
      : [0x15, index & 0xff];

      // 添加前缀
      Uint8List data = Utils.addPrefixToUint8List(prefix, pack);
